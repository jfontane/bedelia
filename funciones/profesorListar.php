...
if($action == 'listar'){
	$tables = " profesor p, persona per ";
	$campos = " p.id, p.dni, p.apellido, p.nombre, per.telefono, per.telefono_caracteristica, per.telefono_numero, per.email ";
	//if ($id) $andX[] = 'c.id = "' . $id .'"';
    if ($nombre) $andX[] = '(p.apellido like "%' . $nombre . '%" or p.nombre like "%' . $nombre . '%")';  
