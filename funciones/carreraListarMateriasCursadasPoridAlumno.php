...
if($action == 'listar' && $idCarrera && ArrayHash::check($hash, array($MY_SECRET=>$idAlumno))){
	...
	$campos = " m.id, m.nombre, m.anio, acm.idTipoCursadoAlumno, acm.anioCursado, acm.nota, acm.estado_final, acm.FechaVencimientoRegularidad, tca.nombre as tipo_cursado ";
		
	if (count($where)>0) $sWhere =' WHERE ' . implode(" and ",$where);
	
	$sWhere .= " ORDER BY acm.anioCursado DESC, m.nombre ASC  ";
				...
				<table class="table table-striped table-bordered table-hover" id="tabla_calendario">
					<thead class="thead-dark">
						<tr>
							...
							<th class="text-center" width="40%"><small><b>MATERIA</b><small></th>
    ...
    while ($row = mysqli_fetch_assoc($query)) {
			...
			$espacio = "&nbsp;";
			$accion_editar = '<a href="#" onclick="cursadoEditar(\''.$idAlumno.'&'.$row['id'].'&'.$row['nombre'].'&'.$row['anioCursado'].'&'.$row['idTipoCursadoAlumno'].'&'.$row['nota'].'&'.$row['estado_final'].'&'.$row['FechaVencimientoRegularidad'].'\')" title="Editar"><img src="../public/img/icons/edit_icon.png" width="20"></a>';
